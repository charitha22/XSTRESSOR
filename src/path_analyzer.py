...
class predicateModel:
    def __init__(self, predicate, seq_data, scale_data):
        ...
        printDbgMsg("building a predicate model for predicate "+str(predicate))
        ...
        if(self.seq_data != None and self.scale_data!=None):
            self.hasScale = True
            self.buildSeqModels()
    ...
    def buildScaleModel(self):
        ...
        self.scale_model = fitLowestRankPolyModel(self.scale_data[0], self.scale_data[1])
    ...
    def buildNonScaledModel(self, scale_to_cond_map):
        #print(scale_to_cond_map)
        scales = list(scale_to_cond_map.keys())
        scales.sort()
        #print(scales)
...
class PredicateMap:
    ...
    def findUniquePredicates(self, longest_path, scale_longest_path):
        ...
        for item in longest_path:
            ...
            if(isinstance(item, str)):
                ...
                # TODO : value_vec is not used here! combine addPathCond with this
    ...
    def addPathCond(self, expr_array, scale):
        ...
        scales = self.pred_to_scale_to_cond_map[str(self.predicates[0])].keys()
        scales.sort()
    ...
    def updateMap(self, predicate,scale,feature_vec):
        ...
        self.pred_to_scale_to_cond_map[str(predicate)][scale].append(feature_vec)
    ...
    def buildPredicateModel(self, predicate):
        ...
        for t in range(0,5):
            ...
            if(self.hasScaleProperties(predicate_hits)):
                ...
                #print(predicate_model.scale_model)

            else:
                #print(scale_to_cond_map)
                #print("This predicate does not scale")
        ...
        for i in range(0, no_of_seq_models):
            ...
            for j in range(0, no_of_models):
                ...
                for k in range(0, len(leaf_models)):
                    max_scale_leaf_models[k].leaf_correction_data.append(int(round(leaf_models[k].models.c[-1])))
    ...
    # if predicate hits is 1 across all scales we decide this predicate doesn't scale
...
class PathAnalyzer:
    ...
    def buildModel(self):
        ...
        printInfoMsg("This program has "+ str(len(self.program.predicates))+" symbolic predicates")
        #print(self.program.pred_to_scale_to_cond_map)
    ...
    def genScaleTest(self, scale):
        ...
        for pmodel in self.predicate_models:
            ...
            for i in range(0, len(ind_var_seqs[0])):
                ...
                cond_writer = ExprWriter(pmodel.predicate, expr_val_vec, self.symbolic_store)
                path_condition.append(cond_writer.condition) 

                
        # path_condition += self.addInitialConditions(scale)
        
        return [path_condition,self.symbolic_store, array_funcs]

        
    
    # this is an ugly hard coded function
    # def addInitialConditions(self,scale):

        # init_conds = []

        # for key in self.symbolic_store:
            # sym_var = self.symbolic_store[str(key)]

            # if("graph" in str(sym_var)):
                # init_conds.append(sym_var>=0)

            # ugly : make the src node distance zero
            # true_scale = scale+2
            # if("dist_"+str(true_scale) in str(sym_var)):
                # init_conds.append(sym_var==0)
