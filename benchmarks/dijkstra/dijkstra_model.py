    ...
    for k in range(0,V):
        ...
        for i in range(0,V):
            if((not fixed[i]) and (dist[minval]+graph[minval][i]<dist[i])):
                ...
                if(k!=0):
                    path_cond.append(sym_store.getSymVar("dist", [minval], False)+sym_store.getSymVar("graph", [minval,i], False) < sym_store.getSymVar("dist", [i], False))
                    
                    # add assignment constraint
                    rhs = sym_store.getSymVar("dist", [minval], False)+sym_store.getSymVar("graph", [minval, i], False)
                    lhs = sym_store.getSymVar("dist", [i], True)
                    path_cond.append(rhs == lhs)

                else:
                    path_cond.append(sym_store.getSymVar("dist", [V], False)+sym_store.getSymVar("graph", [V,i], False) < sym_store.getSymVar("dist", [i], False))
                    
                    # add assignment constraint
                    rhs = sym_store.getSymVar("dist", [V], False)+sym_store.getSymVar("graph", [V, i], False)
                    lhs = sym_store.getSymVar("dist", [i], True)
                    path_cond.append(rhs == lhs)
